...
export async function toggleLike(user_id: string, card_id: string, card_title?: string, savings?: number) {
  const response = await fetch('/api/likes', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ user_id, card_id, card_title, savings }),
  })
  if (!response.ok) throw new Error('Failed to toggle like')
  return response.json()
}

export async function getLikes(_user_id?: string) {
