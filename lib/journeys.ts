...
export type JourneyId = typeof JOURNEY_ORDER[number]
...
export interface JourneyQuestion {
  ...
  type: 'options' | 'number'
  options?: string[]
  /** Softer copy when re-asking numeric (0/empty/implausible); one repeat only. */
  repeatLabel?: string
}
...
export function getNextJourney(currentJourneyId: JourneyId | null, completedJourneys: JourneyId[]): JourneyId | null {
  if (!currentJourneyId) {
    return JOURNEY_ORDER[0]
  }

  const currentIndex = JOURNEY_ORDER.indexOf(currentJourneyId)
  if (currentIndex === -1) return null

  for (let i = currentIndex + 1; i < JOURNEY_ORDER.length; i++) {
    if (!completedJourneys.includes(JOURNEY_ORDER[i])) {
      return JOURNEY_ORDER[i]
    }
  }
...
export function getAllCompletedJourneys(userId: string): Promise<JourneyId[]> {
