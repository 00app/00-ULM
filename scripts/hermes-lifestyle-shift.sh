...
# Build JSON body (jq when available; else printf — escape answer for JSON)
BODY_TMP="$(mktemp)"
if command -v jq >/dev/null 2>&1; then
  jq -n \
    ...
    + (if ($parent | length) > 0 then {parent_answer_id: $parent} else {} end)' > "$BODY_TMP"
else
  esc_ans="${ANSWER_VALUE//\\/\\\\}"
  esc_ans="${esc_ans//\"/\\\"}"
  printf '{"trigger":true,"source":"hermes-lifestyle-shift","postcode":"%s","category":"%s","question_id":"%s","answer_value":"%s","lifestyle_mode":"shift","is_achievement_card":true' \
    "$POSTCODE" "$CATEGORY" "$QUESTION_ID" "$esc_ans" > "$BODY_TMP"
...
while [[ "$attempt" -le "$MAX_RETRIES" ]]; do
  ...
  if [[ -s "$RESP" ]]; then
    if command -v jq >/dev/null 2>&1; then
      if [[ "$HTTP_CODE" == "200" || "$HTTP_CODE" == "201" || "$HTTP_CODE" == "202" ]]; then
        jq '{ok,mode,lifestyle_mode,is_achievement_card,url_shield,achievement,achievement_error,achievement_card:{id,title,journey_key}}' "$RESP" 2>/dev/null || cat "$RESP"
      else
        echo "  Response:" >&2
  ...
  if [[ "$RETRY" -eq 0 ]]; then
    echo "✓ Lifestyle shift research complete (URL shield passed or no retry flag)."
    rm -f "$BODY_TMP"
    exit 0
  fi
