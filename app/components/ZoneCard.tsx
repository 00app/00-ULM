...
/** Ping & Fade: content feeds in 150ms after container ping */
const CONTENT_REVEAL_DELAY_MS = 150
const CONTENT_REVEAL_DURATION_MS = 450
const CONTENT_REVEAL_EASE = [0.16, 1, 0.3, 1] as const
...
/** Money-first: CTA leads with the saving (Get this Saving / Check Deal) */
function getActionLabel(actions?: ZoneCardItem['actions']): string {
  if (actions?.actionType === 'learn' || actions?.actionType === 'view') return 'Check Deal'
  return 'Get this Saving'
}
    ...
    >
      {isExpanded ? (
        <>
          ...
          {/* Top: Journey pill — no border (visual lock) */}
          <div
            style={{
          ...
          {/* Middle: Expanded content — Ping & Fade: translateY(15px) → 0 after ping */}
          <motion.div
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: contentVisible ? 1 : 0, y: contentVisible ? 0 : 15 }}
            transition={{ duration: CONTENT_REVEAL_DURATION_MS / 1000, ease: CONTENT_REVEAL_EASE, delay: contentVisible ? 0.15 : 0 }}
            style={{
              flex: 1,
              display: 'flex',
              flexDirection: 'column',
          ...
          {/* Bottom: 3-button row + CLOSE — same fade-up */}
          <motion.div
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: contentVisible ? 1 : 0, y: contentVisible ? 0 : 15 }}
            transition={{ duration: CONTENT_REVEAL_DURATION_MS / 1000, ease: CONTENT_REVEAL_EASE, delay: contentVisible ? 0.15 : 0 }}
        ...
        /* Collapsed: minimal preview — no border (visual lock) */
        <>
          <div
            style={{
              alignSelf: 'flex-start',
