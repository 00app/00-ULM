...
export default function ZoneBentoCard({ item, variant, isExpanded, onTap, onClose }: ZoneBentoCardProps) {
  ...
  const actionLabel = (item.actions?.actionType === 'learn' || item.actions?.actionType === 'view' ? 'Check Deal' : 'Get this Saving')
  ...
  return (
    ...
    >
      ...
      {isExpanded && (
        <>
          <ZeroLogoIcon className="zone-card-expanded-logo" style={{ color: 'var(--color-purple)' }} />
