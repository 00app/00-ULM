...
- **Solo Focus trinity:** Close (X) → lifestyle loop; **like** → card stays open (selected state) + `/likes` snapshot; **nope** → `OfferFeedbackTakeover` then grid. App chrome icons use **2px** mono stroke (`monoStrokeWidth` in `MonoStrokeIcons.tsx`).
