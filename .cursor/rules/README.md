# Rules Index

This workspace uses:

- `postcode-first-architect.mdc` — global engine governance: dynamic postcode, 12k/1t truth, UI ceilings, deploy pipeline, Zai palette (always on).
- `zero-zero-prime-directive.mdc` — universal manifest: motion DNA, intelligence loop, UI constraints (always on).
- `mechanical-pulse.mdc` — design, typography, spacing, and colour lock for Zone / Solo Focus.
- `zone-voice-copy.mdc` — Forensic Mate written voice, nav labels, banned phrases, SAVE not SAVINGS (`lib/zone/zoneVoice.ts`).
- `verify-deploy-gate.mdc` — lint/typecheck verify before commit or deploy; Vercel `vercel-build-gate` pipeline (always on).

Logic contracts and Sentinel economics live in **`lib/logic/engine.ts`** (v21 consolidated engine).

**Canonical doc:** `docs/GUARDRAILS-AND-PIPELINE.md` — pipeline + guardrail stack + maintenance workflow.

Use this file as the quick index; keep behavioural detail in `mechanical-pulse.mdc` and the engine module.
