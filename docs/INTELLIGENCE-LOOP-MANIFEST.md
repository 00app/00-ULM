...
- **Expanded Solo Focus:** **Marvin** hook H1 (**10–20 words**, 2–3 lines via **`headlineFromExpandedHook`**) + **Marvin H4 lead** (town from **`locationState.locationName`**, not postcode) + two **Roboto** body paragraphs + payoff. Zone bento: **5–8 words** via **`headlineFromTitle`**. Voice: **`lib/zone/zoneVoice.ts`**. Gemini article-tier triplet in **`lib/agents/researchAgent.ts`** → **`architect_prose`** + **`expanded_headline`** on new rows.
