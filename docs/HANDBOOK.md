...
| Layout | Shared mother stack: **`SoloFocusMotherStack`** (category → H1 → prose → metrics + inset **`SoloFocusJourneyNav`**). Journey nav uses **wall-aware** ring (`soloFocusJourneyRing` on Zone). Expanded: **Marvin H3** headline + three **Roboto Bold** prose paragraphs (≤ 40 words each). Shell padding **20px**; inset nav **0** horizontal padding. |
| Ask Zai | **`AskZaiDeepDiveSheet`** (audit trail + category pills) → **Continue in Zai** → `/zai`. Chat is **read-only** (scrape only on **Search deeper**). **`POST /api/zai`** loads **`research_results`** URLs/£ for the active category (not `architect_prose`). ULM chat bubbles: `#FFD700` / `#1A1A1A`. Solo Focus expanded shell: transparent HUD tint (`lib/journeyColors.ts` **`zoneExpandedJourneySurfaceStyleProps`**). **Flow:** [ZAI-AND-QUESTIONS-RULES.md](ZAI-AND-QUESTIONS-RULES.md). Code: `lib/zai/chatBoundaries.ts`, `lib/zai/deepDiveAudit.ts`, `lib/brains/zai/prompts.ts`. |
