...
**Ranked action wall — replaces this ordering for profiled users (2026-08).** Everything above (moneySortedJourneys, JOURNEY_ORDER, one-mother-per-journey) still governs the wall for **guests and partial profiles**. Once a user has answered `financial_pressure`, `buildZoneViewModel` swaps to a completely different selection model: `selectActionsForProfile()` (`lib/actions/selectActions.ts`) ranks a 63-entry hand-tagged, cited library (`lib/actions/actionLibrary.ts`) down to the twelve highest-scoring actions that survive hard exclusions, eligibility (`requires`), and the person's cost ceiling — no LLM call, fully deterministic, same profile in → same twelve out. `actionsToJourneyCards()` (`lib/actions/actionCards.ts`) renders the result through the *same* `ZoneJourneyCard` shape and `JourneyBentoCard` component the category wall already used, so this is a selection-layer swap, not a new UI. A ranked wall can legitimately put three MONEY cards on screen at once — something the old one-per-`journey_key` model assumed could never happen — so cards are addressed by **action id**, not category; `/zone/card/[journeyKey]` resolves both shapes for deep links and the back button. `buildGroovyGridItems` groups by key and walks each group (rather than keying a `Map` on `journey_key`, which silently collapsed same-key ranked cards to one survivor and repeated it down the grid — a real, shipped bug, fixed 2026-08) and shares `MAX_CARDS_PER_CATEGORY` with the ranker's own diversity cap so the two can't disagree on how many of one category should render. Full mechanics, gate model, and the string of follow-up fixes: [GUARDRAILS-AND-PIPELINE.md §2c](GUARDRAILS-AND-PIPELINE.md#2c-ranked-action-library-2026-08--replaces-one-card-per-category).

When the wall is ranked, the **hero** headline and "biggest win" are recomputed as the literal sum/top-card of the twelve shown, rather than the pre-existing `dynamicJourneyValues` sum across all category calculators — those two numbers had drifted apart badly enough on a real profile (hero £4.1k vs a wall summing to £8,688) to read as invented, since neither could be checked against the other. Guests and partial profiles keep the original calculator-summed hero.
...
**Tips row pivoted to the action library for profiled users (2026-08).** The Rock catalog above is 68 hand-written habits of which only 22 carry any profile gate at all, rotated purely on day index and season — it cannot see tenure, money, employment, or children, and its own header calls its figures "indicative annual figures, rounded for UX" (a provider name but no citation, no `verifiedOn`), a different provenance standard from the ranked wall's cited-and-dated values. Running both standards on one screen is what made the whole wall read as guesswork even where it wasn't — the reported symptom was literally "greywater plants, SAVE £0" shown to a renter who'd just said money was tight. `lib/actions/actionTips.ts`'s `selectTipHabits()` now draws the same gated, cited pool `selectActionsForProfile` uses for the wall, filtered to what's actually doable **at this hour** (`timeOfDayForAction` — claims/switches in the afternoon since council and supplier phone lines are shut by evening; home/utilities/water in the evening; travel in the morning) and excluding zero-£ actions specifically for this three-slot strip (a £0 card can still earn a wall slot on carbon/protection value, but "SAVE £0" in a tips row reads as the app having nothing to say) and anything already shown on the wall below, so the strip never echoes the grid. `actionToTipHabit()` adapts a `ZoneAction` into the existing `RockHabit` shape so `RockSavingTips` renders unchanged — a swap of where the content comes from, not a UI rewrite. Guests and partial profiles still fall through to the original Rock rail.
